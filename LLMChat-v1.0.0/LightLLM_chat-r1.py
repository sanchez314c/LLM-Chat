...
class MacOSTTS:
    ...
    def setup_synthesizer(self):
        try:
            self.log_callback(f"Setting up synthesizer with voice {self.voice} on thread {threading.current_thread().name}", "system")
    ...
    async def generate_and_play_audio(self, text):
        ...
        if self.synthesizer is None:
            ...
            return False
        try:
            self.log_callback(f"Generating audio with macOS TTS on thread {threading.current_thread().name}...", "system")
    ...
    def get_available_voices(self):
        if not MACOS_TTS_AVAILABLE or platform.system() != "Darwin":
            return []
        try:
            self.log_callback(f"Fetching available voices on thread {threading.current_thread().name}...", "system")
...
class VoyeurChat(tk.Tk):
    ...
    def update_theme(self):
        ...
        # Update the settings canvas background
        settings_canvas = self.settings_panel.winfo_children()[0]  # The canvas is the first child
        settings_canvas.configure(bg=LEFT_PANEL_BG if self.is_dark_mode.get() else LIGHT_LEFT_PANEL_BG)
    ...
    def _init_ui(self):
        ...
        # Right Panel: Settings with Scrollbar
        ...
        # Create a canvas and scrollbar for the settings panel
        ...
        # Bind the canvas to update its scroll region
        ...
        def _configure_settings_canvas(event):
            canvas_width = event.width
        ...
        settings_canvas.bind("<Configure>", _configure_settings_canvas)

        # Bind mouse wheel scrolling to the settings canvas
        settings_canvas.bind_all("<MouseWheel>", self._on_mousewheel_settings)

        # Use the scrollable frame as the container for settings widgets
        ...
        ttk.Button(self.macos_settings_frame, text="Refresh Voices", command=self.refresh_macos_voices, style="Dark.TButton").pack(fill=tk.X, pady=(0, 10), padx=10)
    ...
    def _on_mousewheel_settings(self, event):
        # Check if the mouse is over the settings panel before scrolling
    ...
    def refresh_macos_voices(self):
        if not hasattr(self, 'macos_tts') or self.macos_tts is None:
