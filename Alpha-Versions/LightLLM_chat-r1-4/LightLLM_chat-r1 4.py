...
from datetime import datetime
 # --- Dark Theme Colors ---
DARK_BG = "#1C1C1E"          # BoltAI-like very dark background (adjusted for deeper dark)
MEDIUM_DARK_BG = "#333333"   # For input fields or secondary panels
LIGHT_TEXT = "#E5E5E7"       # Slightly softer white for text to match BoltAI tone
...
BORDER_COLOR = "#2A2A2C"      # Darker, subtler border color to blend with background
...
class VoyeurChat(tk.Tk):
    def __init__(self):
        ...
        self.style.configure("Secondary.TFrame", background=DARK_BG, borderwidth=0)  # Darker to match BoltAI sidebar
        self.style.configure("Secondary.Dark.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("SF Pro Display", 11))
        self.style.configure("Dark.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("Arial", 10))
        self.style.configure("MainDark.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("SF Pro Display", 11))
        self.style.configure("Section.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("SF Pro Display", 12, "bold"))

        # Button Style
        self.style.configure("Dark.TButton", background=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, bordercolor=BORDER_COLOR, borderwidth=0, relief=tk.FLAT, padding=4)
        ...
        self.style.configure("Dark.TMenubutton", background=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, bordercolor=BORDER_COLOR, borderwidth=0, relief=tk.FLAT, arrowcolor=LIGHT_TEXT, padding=3)
        self.style.map("Dark.TMenubutton", background=[('active', '#555555')])

        # Scale Style - Ensure background matches parent frame
        self.style.configure("Secondary.Dark.Horizontal.TScale", background=DARK_BG, troughcolor=BORDER_COLOR, sliderrelief=tk.FLAT, sliderthickness=10, bordercolor=BORDER_COLOR)
        self.style.map("Secondary.Dark.Horizontal.TScale", background=[('active', MEDIUM_DARK_BG)], troughcolor=[('active', SELECT_BG_COLOR)])

        # Entry Style
        self.style.configure("Dark.TEntry", fieldbackground=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, bordercolor=BORDER_COLOR, borderwidth=0, relief=tk.FLAT, insertcolor=LIGHT_TEXT, padding=3)
    ...
    def _init_ui(self):
        ...
        # Menu
        # Note: Standard tk.Menu styling is limited; aiming for BoltAI-like dark menu
        ...
        main_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)  # Reduced padding for tighter layout

        # Far-Left Pane: Chat List Panel
        chat_list_panel = ttk.Frame(main_frame, width=220, style="Secondary.TFrame")
        ...
        ttk.Label(chat_list_panel, text="Conversations", style="Section.TLabel").pack(pady=(10, 5), anchor="w", padx=10)
        # Search bar for conversations
        self.search_entry = ttk.Entry(chat_list_panel, style="Dark.TEntry", font=("SF Pro Display", 11))
        ...
        self.chat_listbox = tk.Listbox(chat_list_panel, height=30, bg=MEDIUM_DARK_BG, fg=LIGHT_TEXT,
                                        ...
                                        relief=tk.FLAT, bd=0, highlightthickness=0, exportselection=False)
        ...
        ttk.Button(chat_list_panel, text="New Chat", command=self.create_new_conversation, style="Dark.TButton").pack(fill=tk.X, pady=(5, 10), padx=10, side=tk.BOTTOM)  # Moved to bottom

        # Center Pane: Chat Display and Input Area
        center_panel = ttk.Frame(main_frame, style="MainDark.TFrame")
        center_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 0))  # Reduced padding for integration
        self.chat_history = scrolledtext.ScrolledText(center_panel, wrap=tk.WORD, state=tk.DISABLED,
                                                      ...
                                                      font=("SF Pro Display", 13), padx=8, pady=8, selectbackground=SELECT_BG_COLOR)
        self.chat_history.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        self.chat_history.tag_configure("user", foreground=ACCENT_COLOR_USER, font=("SF Pro Display", 13, "bold"))
        self.chat_history.tag_configure("assistant", foreground=ACCENT_COLOR_ASSISTANT, font=("SF Pro Display", 13))
        self.chat_history.tag_configure("system", foreground=SYSTEM_TEXT, font=("SF Pro Display", 13))
        self.chat_history.tag_configure("error", foreground=ERROR_TEXT, font=("SF Pro Display", 13, "bold"))

        input_area_frame = ttk.Frame(center_panel, style="MainDark.TFrame") # Parent is MainDark
        input_area_frame.pack(fill=tk.X)
        self.user_input = scrolledtext.ScrolledText(input_area_frame, height=2,
                                                    bg=MEDIUM_DARK_BG, fg=LIGHT_TEXT, # User input field itself
                                                    insertbackground=LIGHT_TEXT, 
                                                    wrap=tk.WORD, relief=tk.SOLID,
                                                    bd=1, highlightthickness=0,
                                                    font=("SF Pro Display", 13), padx=8, pady=5, selectbackground=SELECT_BG_COLOR)
        self.user_input.pack(side=tk.LEFT, fill=tk.X, expand=True, pady=(5, 5), padx=(5, 5))
        # Add placeholder text for input
        self.user_input.insert("1.0", "Send a message (Shift+↵ for new line)")
        ...
        self.send_button = ttk.Button(input_area_frame, text="▶", width=3, command=self.send_message, style="Dark.TButton")
        self.send_button.pack(side=tk.RIGHT, fill=tk.Y, pady=(5, 5), padx=(5, 5))
        ...
        settings_panel.pack(side=tk.RIGHT, fill=tk.Y, padx=(0, 0))  # Reduced padding for integration
        settings_panel.pack_propagate(False)
        
        controls_frame = settings_panel # Alias, parent is Secondary.TFrame

        # Section title styling for better BoltAI-like organization
        self.style.configure("Section.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("SF Pro Display", 12, "bold"))

        # MODEL SELECTION SECTION
        ttk.Label(controls_frame, text="MODEL", style="Section.TLabel").pack(pady=(15,5), anchor="w", padx=15)
        ...
        self.model_menu.pack(fill=tk.X, pady=5, padx=15)
        self.model_var.trace_add("write", self.on_model_change)

        # Add "Refresh Models" button in the model section
        ttk.Button(controls_frame, text="Refresh Models", command=self.refresh_models, style="Dark.TButton").pack(fill=tk.X, pady=(0,10), padx=15)

        # CHAT MODE SECTION
        ttk.Label(controls_frame, text="CHAT MODE", style="Section.TLabel").pack(pady=(15,5), anchor="w", padx=15)
        ttk.Label(controls_frame, text="Personality:", style="Secondary.Dark.TLabel").pack(pady=(5,0),anchor="w",padx=15)
        ...
        self.chat_mode_menu.pack(fill=tk.X, pady=5, padx=15)
        self.current_chat_mode.trace_add("write", self.on_chat_mode_change)

        ttk.Label(controls_frame, text="Temperature:", style="Secondary.Dark.TLabel").pack(pady=(10,0),anchor="w",padx=15)
        ...
        self.temp_slider.pack(fill=tk.X, pady=2, padx=15)
        self.temp_display = ttk.Label(controls_frame, text=f"{self.temperature_var.get():.2f}", style="Secondary.Dark.TLabel")
        self.temp_display.pack(pady=(0,5), padx=15, anchor="e")  # Aligned right for BoltAI-like layout
    ...
    def add_log_message(self, message_text, level="system"): # Renamed param
        ...
        # Update conversation log for API context
    ...
    def process_ai_response(self):
        ...
        try:
            ...
            # Note: If streaming causes timing/UI issues, set stream=False for testing
            ...
            # Safeguard against repetitive responses
            if full_assistant_response and len(self.conversation_log) > 0 and self.conversation_log[-1].get('role') == 'assistant':
                last_assistant_response = self.conversation_log[-1].get('content', '')
                if full_assistant_response.strip() == last_assistant_response.strip():
                    print(f"[DEBUG process_ai_response] Skipping repetitive Assistant response")
                    return
            # Avoid duplicate display in UI; response is already streamed to chat_history
            if full_assistant_response and self.current_conversation_id:
                # Only log to database, not UI again
                add_message_to_db(self.current_conversation_id, "assistant", full_assistant_response)
