...
class VoyeurChat(tk.Tk):
    def __init__(self):
        ...
        # For Labels, background should match their parent Frame.
        # Style for labels on MEDIUM_DARK_BG (like side panels):
        self.style.configure("Secondary.Dark.TLabel", background=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, font=("Arial", 10))
        # Style for labels on DARK_BG (if any directly on such a frame):
        self.style.configure("Dark.TLabel", background=DARK_BG, foreground=LIGHT_TEXT, font=("Arial", 10))
        # Style for labels on MainDark.TFrame (like center_panel, if it has direct labels):
        self.style.configure("MainDark.TLabel", background="#1E1E1E", foreground=LIGHT_TEXT, font=("Arial", 10))
        # Button Style
        self.style.configure("Dark.TButton", background=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, relief=tk.FLAT, borderwidth=1, padding=5)
        ...
        self.style.configure("Dark.TMenubutton", background=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, relief=tk.FLAT, arrowcolor=LIGHT_TEXT, borderwidth=1, padding=5)
        # Scale Style - Ensure background matches parent frame
        self.style.configure("Secondary.Dark.Horizontal.TScale", background=MEDIUM_DARK_BG, troughcolor=DARK_BG, sliderrelief=tk.FLAT)
        # Entry Style
        self.style.configure("Dark.TEntry", fieldbackground=MEDIUM_DARK_BG, foreground=LIGHT_TEXT, relief=tk.FLAT, insertcolor=LIGHT_TEXT, borderwidth=1, padding=5)
    ...
    def _init_ui(self):
        ...
        self.system_prompt_text_widget = scrolledtext.ScrolledText(controls_frame, height=6, wrap=tk.WORD, 
                                                                  ...
                                                                  relief=tk.FLAT, font=("Arial", 10), selectbackground=SELECT_BG_COLOR)
